...
export default function App() {
  ...
  const [profile, setProfile] = useState({ name: "", experience: "", platform: "", instrument: "", challenge: "" });
  ...
  const onboarding = [
    ...
    },
    {
      ...
      valid: profile.challenge,
      content: (
        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
          {CHALLENGES.map(c=>(
            <button key={c} onClick={()=>setProfile({...profile,challenge:c})} style={{ padding:"13px 16px", textAlign:"left", background:profile.challenge===c?"#FFD16612":"#0a0d14", border:`1px solid ${profile.challenge===c?"#FFD166":"#111827"}`, borderRadius:8, color:profile.challenge===c?"#FFD166":"#6B7280", fontSize:14, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", transition:"all 0.15s" }}>{c}</button>
          ))}
        </div>
      ),
    },
      ...
      {activeTab==="rules" && (
        <div style={{ flex:1, overflowY:"auto", padding:18 }}>
          ...
          <div style={{ padding:"16px", background:"#FFD16608", border:"1px solid #FFD16618", borderRadius:10, marginTop:8 }}>
            ...
            <div style={{ fontSize:14, color:"#D1D5DB", lineHeight:1.6 }}>{profile.challenge}</div>
          </div>
