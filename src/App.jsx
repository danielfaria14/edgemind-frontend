...
export default function App() {
  ...
  const startCoach = async () => {
    ...
    // Register user with backend and send WhatsApp welcome
    try {
      const rulesPayload = {};
  ...
  if (screen === "success") return (
    <div style={{ minHeight:"100vh", background:"#060810", display:"flex", alignItems:"center", justifyContent:"center", padding:24, fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div className="up" style={{ width:"100%", maxWidth:480, textAlign:"center" }}>
        <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:22, color:"#FFD166", letterSpacing:2, marginBottom:40 }}>EDGEMIND</div>
        <div style={{ fontSize:64, marginBottom:24 }}>📱</div>
        <h2 style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:36, color:"#F9FAFB", letterSpacing:1, marginBottom:12 }}>CHECK YOUR WHATSAPP</h2>
        <p style={{ fontSize:16, color:"#4B5563", lineHeight:1.8, marginBottom:32 }}>
          Your coach just introduced itself on WhatsApp.<br/>
          From now on, all coaching happens there.
        </p>
        <div style={{ padding:"20px 24px", background:"#0a0d14", border:"1px solid #111827", borderRadius:12, marginBottom:24, textAlign:"left" }}>
          <div style={{ fontSize:11, color:"#FFD166", fontWeight:700, letterSpacing:"0.08em", marginBottom:12 }}>HOW IT WORKS</div>
          {[
            ["🌅", "8:00 AM", "Coach sends your morning briefing"],
            ["📊", "After session", "You send: Done. Made $X. Rules: Yes/No"],
            ["🎯", "10:35 AM", "Coach checks in on your session"],
            ["🌙", "7:00 PM", "Evening reflection message"],
          ].map(([icon, time, desc]) => (
            <div key={time} style={{ display:"flex", gap:12, alignItems:"center", marginBottom:10 }}>
              <span style={{ fontSize:18 }}>{icon}</span>
              <div>
                <span style={{ fontSize:12, color:"#FFD166", fontWeight:700 }}>{time}</span>
                <span style={{ fontSize:12, color:"#4B5563", marginLeft:8 }}>{desc}</span>
              </div>
            </div>
          ))}
        </div>
        <div style={{ padding:"14px", background:"#FFD16608", border:"1px solid #FFD16620", borderRadius:8, fontSize:13, color:"#6B7280", lineHeight:1.6, marginBottom:28 }}>
          Save <strong style={{ color:"#FFD166" }}>+1 415 523 8886</strong> as "EdgeMind Coach" in your contacts so you never miss a message.
        </div>
        <button onClick={()=>setScreen("dashboard")} style={{ padding:"12px 32px", background:"transparent", border:"1px solid #111827", borderRadius:8, color:"#374151", fontSize:13, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>
          View Dashboard →
        </button>
      </div>
    </div>
